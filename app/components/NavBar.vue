...
<template>
  <div class="bg-slate-50 dark:bg-gray-900 px-6 py-4 sticky top-0 w-full z-50 shadow-xl">
    <div class="flex justify-between items-center w-full max-w-6xl mx-auto gap-4">
      ...
      <div class="flex lg:border-s gap-4">
        <a class="ms-5 rounded-md px-3 py-2 hover:bg-gray-700 hover:text-white" role="button" title="Toogle Theme" @click="toggleTheme()">
          <Icon :name="colorMode.preference === 'dark' ? 'solar:moon-stars-bold' : 'solar:sun-2-bold'" size="1.5rem" />
        </a>
        <Transition name="slide-right" mode="out-in">
          <button v-if="!isOpen" class="lg:hidden rounded-md px-3 py-2 hover:bg-gray-700 hover:text-white" @click="toggleMenu">
  ...
  <Transition name="slide-left" mode="out-in">
    <div v-if="isOpen" class="w-80 bg-slate-50 dark:bg-gray-900 shadow-xl fixed inset-y-0 left-0 z-50 transform transition-transform lg:hidden" :class="isOpen ? 'translate-x-0' : '-translate-x-full'">
      <div class="flex items-center justify-between border-b dark:border-gray-700 p-4">
        <h2 class="text-lg font-semibold">{{ SITE.name }}</h2>
        <button class="focus:outline-none hover:bg-red-400 hover:text-gray-900 rounded-md p-1" @click="toggleMenu">
