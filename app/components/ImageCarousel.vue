...
<template>
  <div class="group">
      ...
      <div class="relative">
        <div class="flex transition-transform duration-500" :style="{ transform: `translateX(-${currentIndex * 100}%)` }">
          <div v-for="(image, index) in images" :key="index" class="w-full flex-shrink-0">
            ...
            <div v-if="image.alt" class="absolute bottom-0 w-full text-center py-1 bg-slate-700 bg-opacity-75 hidden group-hover:block">
              <span>{{ image.alt }}</span>
            </div>
          </div>
        </div>
      </div>
      <div v-if="images.length > 1" class="absolute top-1/2 transform -translate-y-1/2 flex justify-between w-full px-4">
    ...
    <div v-if="images.length > 1" class="flex justify-center py-1 bg-slate-700 relative z-10">
      <span
        v-for="(image, index) in images"
        :key="index"
        class="h-2 w-2 rounded-full mx-1 cursor-pointer"
        :class="currentIndex === index ? 'bg-slate-200' : 'bg-gray-500'"
        :title="image.alt"
        @click="goTo(index)"
      />
    </div>
